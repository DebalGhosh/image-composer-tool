...
export function BuildImagePage({
  ...
  // Same pattern as Interactive's right-pane collapse, reflected around the
  // divider so it's the LEFT pane that slides away to 0% width. The rAF
  // machinery lives in hooks/usePanelAnimation — it was duplicated here, in
  // InteractivePage and (inlined) in BasicPage.
  const { panelRef: leftPanelRef, animateTo: animatePanel } = usePanelAnimation()
  ...
  }, [collapsed, animatePanel, leftPanelRef])

  // No unmount cleanup here: usePanelAnimation cancels its own in-flight frame.
