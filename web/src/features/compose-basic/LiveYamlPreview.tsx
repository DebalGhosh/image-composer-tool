...
import { useComposedYaml } from './hooks/useComposedYaml'
...
export function LiveYamlPreview({ selection, complete }: LiveYamlPreviewProps) {
  ...
  // The debounced, abortable compose. `beat` is the animation key; the effect's
  // exhaustive-deps suppression is load-bearing. See hooks/useComposedYaml.
  const { yaml, loading, error, beat } = useComposedYaml({ selection, complete })
  /* ------------------------- render ------------------------- */
