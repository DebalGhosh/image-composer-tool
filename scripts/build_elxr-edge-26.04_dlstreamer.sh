...
build_elxr_edge_26_04_dlstreamer_image() {
  echo "Building ELXR Edge 26.04 DLStreamer Image. (using image-composer-tool binary)"
  echo "Ensuring we're in the working directory before starting builds..."
  cd "$WORKING_DIR"
  echo "Current working directory: $(pwd)"
  
  set +e
  output=$(sudo -S ./image-composer-tool build image-templates/elxr-edge-26.04-x86_64-dlstreamer.yml 2>&1)
  build_exit_code=$?
  set -e
  
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "ELXR Edge 26.04 DLStreamer Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ELXR Edge 26.04 DLStreamer image..."
      if run_qemu_boot_test "elxr-edge-26.04-dlstreamer-x86_64"; then
        echo "QEMU boot test PASSED for ELXR Edge 26.04 DLStreamer image"
      else
        echo "QEMU boot test FAILED for ELXR Edge 26.04 DLStreamer image"
        exit 1
      fi
      cleanup_image_files raw
    fi
  else
    echo "ELXR Edge 26.04 DLStreamer Image build failed."
    echo "Build output:"
    echo "$output"
    exit 1
  fi
